...
from ..storage import delete_from_s3, list_old_files
...
def cleanup_old_files() -> Dict[str, Any]:
    ...
    try:
        ...
        old_events = (
            db.query(SecurityEvent).filter(SecurityEvent.created_at < cutoff_date).all()
        )
...
def cleanup_failed_uploads() -> Dict[str, Any]:
    ...
    try:
        ...
        old_files = list_old_files(
            s3_client, settings.s3_bucket_name, "events/", 1
        ...
        for file_key in old_files:
            ...
            try:
                ...
                event_exists = (
                    ...
                    .filter(SecurityEvent.event_id == event_id)
                    .first()
                )
        ...
        result = {"deleted_orphaned_files": deleted_count, "status": "completed"}
