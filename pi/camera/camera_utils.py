...
class CameraManager:
    ...
    def record_low_res_video(self, filename=None):
        ...
        try:
            ...
            if (
                self.video_settings["format"].lower() == "mp4"
                and temp_filename != filename
            ):
                try:
                    import subprocess
                    import os
                    ...
                    if pir_sensor and pir_sensor.wait_for_motion(timeout=10):
                        ...
                        if self.camera_busy.is_set():
                            ...
                            if consecutive_busy_count <= max_consecutive_busy:
                                print(
                                    f"Camera Thread: Camera busy, skipping motion event ({consecutive_busy_count}/{max_consecutive_busy})"
                                )
                            elif consecutive_busy_count == max_consecutive_busy + 1:
                                print(
                                    "Camera Thread: Camera busy for too long, will force reset soon..."
                                )
                            elif consecutive_busy_count >= max_consecutive_busy + 5:
                                print(
                                    "Camera Thread: Forcing camera reset due to extended busy state"
                                )
                                self.camera_busy.clear()
                                consecutive_busy_count = 0
                            continue
