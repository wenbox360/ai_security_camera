...
class BehaviorAnalyzer:
    ...
    def __init__(self):
        ...
        self.frame_skip = (
            Settings.get_video_frame_skip()
        )  # Analyze every nth frame for efficiency
        self.min_confidence = (
            Settings.get_min_person_confidence()
        )  # Minimum YOLO confidence for person detection
    ...
    def _analyze_video_file(self, video_path, yolo_handler):
        ...
        try:
            ...
            return self._create_error_result(
                "OpenCV not available for video analysis", "Missing cv2 dependency"
            )
        ...
        if not cap.isOpened():
            return self._create_error_result(
                "Could not open video file", "Video file access failed"
            )
        ...
        try:
            ...
            if total_frames > 0 and total_frames <= 100000:  # Reasonable range
                ...
                print(
                    f"❌ Invalid frame count: {total_frames}, will calculate dynamically"
                )
            ...
            if frame_count_reliable:
                print(
                    f"Duration: {video_duration:.1f}s, FPS: {fps:.1f}, Frames: {total_frames}"
                )
            else:
            ...
            cap.release()
            return self._create_error_result(
                f"Error reading video properties: {str(e)}", str(e)
            )
        ...
        # Calculate actual video duration from manually counted frames
        if not frame_count_reliable or video_duration <= 0:
            actual_video_duration = frame_count / fps
            print(
                f"📊 Calculated actual duration: {actual_video_duration:.1f}s from {frame_count} frames"
            )
        else:
    ...
    def _analyze_dwelling_patterns(
        self,
        person_detections,
        video_duration,
        frames_with_people,
        ...
        people_presence_time = (
            len(person_detections) * self.frame_skip / 30
        )  # Estimate based on frame analysis
        presence_percentage = (
            (frames_with_people / total_analyzed_frames) * 100
        ...
        longest_presence = (
            max(continuous_presence_periods) if continuous_presence_periods else 0
        )
        ...
        if len(person_detections) >= 5 and video_duration >= 2:
            detection_spread = self._calculate_detection_spread(
                person_detections, video_duration
            )
        ...
        avg_people_count = sum(d["people_count"] for d in person_detections) / len(
            person_detections
        )
        ...
        return {
            ...
            "message": self._generate_dwelling_message(
                dwelling_detected,
                longest_presence,
                presence_percentage,
                avg_people_count,
    ...
    def _generate_dwelling_message(
        self, detected, longest_presence, presence_percentage, avg_people
    ):
