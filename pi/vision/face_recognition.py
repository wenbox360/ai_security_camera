...
class FaceRecognitionHandler:
    ...
    def _save_known_faces(self):
        ...
        try:
            ...
            print(
                f"Saved {len(self.known_faces)} known faces to {self.embeddings_file}"
            )
    ...
    def store_face_from_image(self, image_data, person_name, person_id=None):
        ...
        try:
            ...
            try:
                ...
                image_filename = (
                    f"{person_id}_{len(self.known_faces[person_id]['embeddings'])}.jpg"
                )
            ...
            return {"success": False, "error": str(e), "person_id": None}

    def store_face_embedding(self, embedding, person_name, person_id=None):
        """Persist a validated 128-value face embedding received from the cloud."""
        try:
            face_embedding = np.asarray(embedding, dtype=float)
            if face_embedding.ndim != 1 or face_embedding.size != 128:
                return {
                    "success": False,
                    "error": "Face embedding must contain exactly 128 numeric values",
                    "person_id": None,
                }

            person_id = person_id or person_name.lower().replace(" ", "_").replace(
                "-", "_"
            )
            existing = self.known_faces.get(person_id, {})
            self.known_faces[person_id] = {
                "name": person_name,
                "embeddings": [face_embedding],
                "created_date": existing.get(
                    "created_date", datetime.now().isoformat()
                ),
            ...
            return {"success": False, "error": str(exc), "person_id": None}
