"""Raspberry Pi hardware smoke tests (run on a configured device)."""
...
def test_imports():
    ...
    try:
        modules = [
            ...
            ("pi.utils.security_logger", "Security Logger"),
            ("pi.vision.yolo_handler", "YOLO Handler"),
            ("pi.vision.face_recognition", "Face Recognition"),
            ("pi.inference.behavior_analyzer", "Behavior Analyzer"),
            ("pi.utils.config_queue", "Configuration Queue"),
            ("pi.camera.camera_utils", "Camera Manager"),
            ("pi.sensors.pir", "PIR Sensor"),
        ]
...
def test_logger():
    ...
    try:
        ...
        # Test logging
        logger.log_security_event("test_event", {"message": "System test"}, "INFO")
...
def test_yolo_handler():
    ...
    try:
        from pi.vision.yolo_handler import YOLOHandler
...
def test_face_recognition():
    ...
    try:
        from pi.vision.face_recognition import FaceRecognitionHandler
...
def test_behavior_analyzer():
    ...
    try:
        from pi.inference.behavior_analyzer import BehaviorAnalyzer
...
def test_config_queue():
    ...
    try:
        from pi.utils.config_queue import ConfigurationQueue, ConfigAction
        ...
        request_id = config_queue.add_request(
            ConfigAction.UPDATE_YOLO_CONFIG, {"min_confidence": 0.8}, priority=1
        )
...
def main():
    ...
    print("4. Run the main system: python3 -m pi.main")
