...
class ConfigAction(Enum):
    ...
    ADD_TRUSTED_EMBEDDING = "add_trusted_embedding"
...
class ConfigurationQueue:
    ...
    def add_request(
        self, action: ConfigAction, data: Dict[str, Any], priority: int = 1
    ) -> str:
        """
        ...
        try:
            ...
            self.config_queue.put(
                (priority, self.request_counter, config_request), timeout=1
            )
    ...
    def _process_queue(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                _priority, _sequence, config_request = self.config_queue.get(timeout=1)

                self.is_processing = True
                print(
                    f"🔧 Processing config request: {config_request.action.value} (ID: {config_request.request_id})"
                )
                ...
                if success:
                    ...
                    print(
                        f"❌ Config request failed: {config_request.request_id} - {result}"
                    )
    ...
    def _execute_config_request(self, request: ConfigRequest) -> tuple[bool, str]:
        ...
        try:
            ...
            elif action == ConfigAction.ADD_TRUSTED_EMBEDDING:
                return self._add_trusted_embedding(data)
    ...
    def _update_yolo_config(self, data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            yolo_handler = (
                self.security_system.yolo_handler if self.security_system else None
            )
            ...
            if "min_confidence" in data:
                ...
                if 0.0 <= new_confidence <= 1.0:
                    ...
                    if self.security_system.behavior_analyzer:
                        self.security_system.behavior_analyzer.min_confidence = (
                            new_confidence
                        )
                    results.append(f"min_confidence={new_confidence}")
                else:
            ...
            if "model_path" in data:
                ...
                if os.path.exists(model_path):
                    results.append(f"model_path={model_path} (restart required)")
                else:
                    return False, f"Model file not found: {model_path}"

            if not results:
                return False, "No supported YOLO settings provided"

            return True, f"YOLO config updated: {', '.join(results)}"
    ...
    def _add_trusted_face(self, data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            face_handler = (
                self.security_system.face_recognition if self.security_system else None
            )
            ...
            # Store the face
            result = face_handler.store_face_from_image(image_data, name)

            if result.get("success"):
                return True, f"Trusted face added: {name}"
            else:
                return False, result.get("error", f"Failed to add face: {name}")
    ...
    def _remove_trusted_face(self, data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            ...
            matches = [
                person_id
                for person_id, face_data in face_handler.known_faces.items()
                if person_id == name or face_data.get("name") == name
            ]
            if not matches:
                return False, f"Face not found: {name}"

            for person_id in matches:
                del face_handler.known_faces[person_id]
            face_handler._save_known_faces()
            return True, f"Trusted face removed: {name}"

        except Exception as e:
            return False, f"Remove trusted face failed: {str(e)}"

    def _add_trusted_embedding(self, data: Dict[str, Any]) -> tuple[bool, str]:
        """Install a cloud-provided face embedding without treating it as image data."""
        face_handler = (
            self.security_system.face_recognition if self.security_system else None
        )
        if not face_handler:
            return False, "Face recognition handler not available"
        if not data.get("name") or not isinstance(data.get("embedding"), list):
            return False, "Missing required fields: name, embedding"

        result = face_handler.store_face_embedding(
            data["embedding"], data["name"], person_id=data.get("person_id")
        )
        if result.get("success"):
            return True, f"Trusted embedding added: {data['name']}"
        return False, result.get("error", "Unable to add trusted embedding")

    def _update_dwelling_config(self, data: Dict[str, Any]) -> tuple[bool, str]:
        """Update dwelling detection configuration"""
        try:
            analyzer = (
                self.security_system.behavior_analyzer if self.security_system else None
            )
    ...
    def _update_camera_config(self, data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        try:
            camera = (
                self.security_system.camera_manager if self.security_system else None
            )
            ...
            return (
                True,
                f"Camera config updated: {', '.join(results)} (may require restart)",
            )
    ...
    def update_yolo_confidence(self, confidence: float, priority: int = 1) -> str:
        ...
        return self.add_request(
            ConfigAction.UPDATE_YOLO_CONFIG, {"min_confidence": confidence}, priority
        )

    def add_trusted_person(
        self, name: str, image_data: bytes, priority: int = 1
    ) -> str:
    ...
    def add_trusted_embedding(
        ...
        """Queue a trusted embedding received from the cloud settings API."""
        return self.add_request(
            ConfigAction.ADD_TRUSTED_EMBEDDING,
            {"name": name, "embedding": embedding, "person_id": person_id},
            priority,
        )
    ...
    def remove_trusted_person(self, name: str, priority: int = 1) -> str:
        ...
        return self.add_request(
            ConfigAction.REMOVE_TRUSTED_FACE, {"name": name}, priority
        )
    ...
    def get_request_status(self, request_id: str) -> Dict[str, Any]:
        ...
        if request_id in self.completed_requests:
            ...
            return {"status": "failed", "error": self.failed_requests[request_id]}
        else:
